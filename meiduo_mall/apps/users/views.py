...
class HistoryGoodsView(View):
    ...
    def post(self,request):
        ...
        if not request.user.is_authenticated:
            return http.JsonResponse({'code':RETCODE.SESSIONERR,'errmsg':'用户未登入'})
    ...
    def get(self,request):
        ...
        # class 'dict'>: {'id': 3, 'name': 'Apple iPhone 8 Plus (A1864) 64GB 金色 移动联通电信4G手机', 'price': Decimal('6499.00'),
        #                 'default_image_url': 'http://192.168.15.93:8888/group1/M00/00/02/CtM3BVrRZCqAUxp9AAFti6upbx41220032'}
        ...
        #定义一个列表,用来装sku字典数据
        sku_list = []
        #遍历sku_id列表，有顺序的一个一个去获取sku模型并转化成字典
...
class ModificationPasswordView(View):
    """修改用户密码"""
    ...
    def get(self,request,account):
        ...
        if all([account,text,image_code_id]) is False:
            return http.HttpResponseForbidden('缺少必传参数')
        # 创建redis连接对象
        redis_conn = get_redis_connection('verify_code')
        # 获取redis中图形验证码
        ...
        if image_code_server is None:
            return http.JsonResponse({'code': RETCODE.IMAGECODEERR, 'errmsg': '图形验证码已过期'},status=400)

        # 判断用户输入的图形验证码和redis中之前存储的验证码是否一致
        # 从redis获取出来的数据都是bytes类型
        if text.lower() != image_code_server.decode().lower():
            return http.JsonResponse({'code': RETCODE.IMAGECODEERR, 'errmsg': '图片验证码输入错误'},status=400)
            # return http.HttpResponseForbidden('验证码不一致')

        user =  get_user_by_account(account)
        # access_token = generate_verify_mobile_url(user)
        serializer = TJW(settings.SECRET_KEY, 3600 * 24)
        data = {'mobile': user.mobile}
        access_token = serializer.dumps(data).decode()

        # redis_conn = get_redis_connection('verify_code')
        # redis_conn.setex(user.id, constants.IMAGE_CODE_EXPIRE_REDIS, image_code_id)
...
class True_Sms_Code_View(View):
    ...
    def get(self,request,account):
        ...
        redis_conn = get_redis_connection('verify_code')
        sms_code_server = redis_conn.get('sms_code_%s' % user.mobile)
        if sms_code != str(sms_code_server.decode()):
